...
export function maskFigmaToCode(context: NodeContext, node: ValidNode, styles: Dict<DeclarationPlain>) {
  ...
  if (mask) {
    ...
    addStyle(styles, '-webkit-mask-image', `/* webpackIgnore: true */ url("${url}")`);
    ...
    const dx = x - node.x;
    const dy = y - node.y;
    addStyle(styles, '-webkit-mask-repeat', `no-repeat`);
    addStyle(styles, 'mask-repeat', `no-repeat`);
    addStyle(styles, '-webkit-mask-position', `${dx}px ${dy}px`);
    addStyle(styles, 'mask-position', `${dx}px ${dy}px`);
    addStyle(styles, '-webkit-mask-size', `${width}px ${height}px`);
