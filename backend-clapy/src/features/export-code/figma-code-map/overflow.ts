...
import { isComponentSet, isGroup, isRectangle, isText, isVector, ValidNode } from '../create-ts-compiler/canvas-utils';
...
export function overflowFigmaToCode(context: NodeContext, node: ValidNode, styles: Dict<DeclarationPlain>) {
  if (isText(node) || isVector(node) || isRectangle(node) || isComponentSet(node) || isGroup(node)) return;
