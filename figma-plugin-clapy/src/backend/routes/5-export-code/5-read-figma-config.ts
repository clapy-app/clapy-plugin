...
export function fillNodesCache<T extends AnyNodeOriginal>(nodeOriginal: T, context: ExtractBatchContext): AnyNode3 {
  ...
  const node2 = { id, type } as AnyNode3;
  ...
  let exportAsSvg =
    (node2 as AnyNode3).visible &&
    !isInstance2(node2) &&
    !isComponent2(node2) &&
    !isPage2(node2) &&
    (isShapeExceptDivable2(node2) || (isBlendMixin(node2) && node2.isMask) || shouldGroupAsSVG(node2));

  if (exportAsSvg) {
    (node2 as AnyNode3).exportAsSvg = exportAsSvg;
  } else {
  ...
  if (isProcessableInstance2(node2, mainComponent)) {
    ...
    if (!componentsCache[id]) {
      ...
      node2.mainComponent = { id, name, type: type as any } as ComponentNode2;
      if (isComponentSet(parent)) {
        node2.mainComponent.parent = parent.id;
      }
    }
  }
...
//  * @param options
//  */
// export async function nodeToObject<T extends AnyNode3>(node: T, extractBatchContext: ExtractBatchContext) {
...
//   return nodeToObjectRec(node, context);
// }
//
// let prevNode: AnyNode | undefined = undefined;

// async function nodeToObjectRec<T extends AnyNode3>(node: T, context: ExtractNodeContext) {
//   try {
//     if (flags.verbose) {
...
function shouldGroupAsSVG(node: AnyNode3) {
...
function isProcessableInstance2(node: AnyNode3, mainComponent?: ComponentNode | null): node is WithCompMixin2 {
